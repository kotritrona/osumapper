/*
    Web version of osu! map file loader
...
function getAllDivData(timeEnd, divisor) {
    // actually es6 (7?) can support writing divisor = 4 in function params, but this is still coooool hahahahaha
    divisor = divisor || 4;
    var uts = uninheritedSections;
    var out = [];
    var ticks = [];
    var timestamps = [];
    var tick = 0;
    ...
    for(let i=0; i<uts.length; i++) {
        var sectionEndTime = (i == uts.length-1) ? timeEnd : uts[i+1].beginTime;
        var beginTick = tick;
        for(let timestamp = uts[i].beginTime; timestamp < sectionEndTime; timestamp += uts[i].tickLength / divisor) {
...
function cutTTD(ttd, groupSize) {
    var lim = ttd.ticks.length - ttd.ticks.length%groupSize;
    ttd.ticks = ttd.ticks.slice(0, lim);
    ttd.timestamps = ttd.timestamps.slice(0, lim);
    ttd.divData = ttd.divData.slice(0, lim);
    return ttd;
}
